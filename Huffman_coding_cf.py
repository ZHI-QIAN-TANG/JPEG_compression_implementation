...
def Huffman_coding(Y1DPCM, Y2DPCM, Y3DPCM, Y4DPCM, CbDPCM, CrDPCM, 
    ...
    def bitstream_to_bytes(bitstream):
        """Convert bitstream to bytes with padding and stuffing"""
    ...
    # 對每個 MCU 進行編碼
    for i in range(len(Y1DPCM)):  # 假設所有数组长度相等
        # 編碼 4 個 Y 區塊
        y_blocks = [(Y1DPCM[i], Y1RLs[i]), (Y2DPCM[i], Y2RLs[i]),
                   (Y3DPCM[i], Y3RLs[i]), (Y4DPCM[i], Y4RLs[i])]
        
        for y_dc, y_ac in y_blocks:
            # DC 編碼
